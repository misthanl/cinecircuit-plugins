    ...
    for catalog_item in registry.builtin_catalog():
        ...
        for field in manifest.config_schema.get("fields") or []:
            assert "type" not in field, (
                f"{manifest.id}.{field.get('key')} 使用了旧 type 声明"
            )
            assert "min" not in field, (
                f"{manifest.id}.{field.get('key')} 使用了旧 min 声明"
            )
            assert "max" not in field, (
                f"{manifest.id}.{field.get('key')} 使用了旧 max 声明"
            )
...
def test_plugin_cron_schedule_matches_five_field_expressions(
    ...
    assert (
        next_cron_time(expression, datetime(2026, 8, 25, 12, 7), timezone_name="UTC")
        == expected
    )


def test_brush_flow_filters_names_and_torrent_sizes() -> None:
    task = {"include": "2160p|4K", "exclude": "CAM", "min_size": 1, "max_size": 10, "promotion": "all"}

    assert SiteTrafficPlugin._matches(
        {"title": "Movie.2160p", "size": 3 * 1024**3}, task
    )
    assert not SiteTrafficPlugin._matches(
        {"title": "Movie.CAM.2160p", "size": 3 * 1024**3}, task
    )
    assert not SiteTrafficPlugin._matches(
        {"title": "Movie.1080p", "size": 3 * 1024**3}, task
    )
    assert not SiteTrafficPlugin._matches(
        {"title": "Movie.2160p", "size": 12 * 1024**3}, task
    )
...
def test_brush_flow_adds_matching_torrents_once() -> None:
    ...
    context = SimpleNamespace(
        ...
        items=SimpleNamespace(
            get=lambda key: recorded.get(key), record=remember,
            list=lambda limit, offset=0, status=None: [
                dict(item_key=key, **item) for key, item in recorded.items()
...
def test_auto_signin_retries_each_matching_site_only_once_per_local_day() -> None:
    ...
    asyncio.run(SiteCheckinPlugin().run(context))
    asyncio.run(SiteCheckinPlugin().run(context))
    assert signer.await_count == 3

    stored["daily_keyword_retries"] = {"day": "1900-01-01", "keys": ["sign:site-1"]}
    final = asyncio.run(SiteCheckinPlugin().run(context))
    assert signer.await_count == 5
...
def test_auto_signin_notifications_use_site_names_and_include_login_results() -> None:
    ...
    assert login_title == "站点任务完成：登录 1/1"
    assert login_body == "登录｜海岸站：登录状态正常"
    ...
    assert {payload["site_name"] for payload in recorded_payloads} == {
        "春天站",
        "海岸站",
    }
...
def test_media_cover_inventory_uses_selectable_servers_and_libraries() -> None:
    context = SimpleNamespace(
        ...
        media_servers=SimpleNamespace(
            ...
            libraries=AsyncMock(
                return_value={"items": [{"id": "movies", "name": "电影"}]}
            ),
        ),
...
@pytest.mark.skipif(
    os.name == "nt" and shutil.which("ffmpeg") is None,
    reason="animated cover encoding requires FFmpeg",
)
@pytest.mark.parametrize(
    ("image_format", "expected"), [("apng", "PNG"), ("gif", "GIF")]
)
def test_cover_renderer_produces_actual_animated_images(
    image_format: str, expected: str, monkeypatch
) -> None:
    import os
    import shutil
...
def test_subtitle_timeline_adjustment_updates_srt_without_touching_other_files(
    tmp_path,
) -> None:
    ...
    result = asyncio.run(
        gateway.adjust_subtitle(str(media), subtitle.name, offset_seconds=1.5)
    )
...
def test_subtitle_plugin_enforces_configured_timeline_limit() -> None:
    ...
    adjust.assert_awaited_once_with(
        "/media/movie.mkv", "movie.zh-CN.srt", offset_seconds=1.25
    )
    with pytest.raises(ValueError, match="30 秒"):
        asyncio.run(
            SubtitleWorkspacePlugin().handle_api(
                PluginApiRequest(
                    action="adjust", method="POST", payload={"offset_seconds": 31}
                ),
                context,
            )
        )
