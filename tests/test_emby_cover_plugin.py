...
def test_media_server_cover_api_keeps_credentials_in_host_gateway() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.method == "POST" and request.url.path == "/emby/Items/library-1/Images/Primary":
...
def test_event_generation_processes_only_located_libraries(monkeypatch) -> None:
    processed = []
        ...
        async def libraries(self, server_id):
            ...
            return {
                "items": [
                    ...
                    {"id": "series", "name": "剧集", "collection_type": "tvshows"},
                ]
            }

    context = SimpleNamespace(
        config={"selected_servers": ["emby-1"], "dry_run": True},
        media_servers=MediaServers(),
        logger=logging.getLogger("test.emby-cover.targeted"),
    )
    plugin = LibraryArtworkPlugin()
    ...
    async def record_library(_run, library):
        processed.append(library["id"])

    monkeypatch.setattr(plugin, "_ensure_cjk_font", skip_font_download)
    monkeypatch.setattr(cover_plugin_module.ArtworkGenerationRun, "_process_library", record_library)

    run = plugin.generation_run(
        context,
        target_libraries={"emby-1": {"series"}},
    )
    result = asyncio.run(run.run())

    assert processed == ["series"]
...
def test_emby_cover_font_is_downloaded_once_to_persistent_cache(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        cover_plugin_module,
        "data_directory",
        lambda namespace: tmp_path / "plugins" / namespace,
    )
    monkeypatch.setattr(cover_plugin_module, "http_client", client)
...
def test_emby_cover_plugin_is_registered_with_explicit_write_permission() -> None:
    ...
    assert fields["trigger_event"]["input_type"] == "select"
    ...
    assert "transfer_monitor" not in fields
