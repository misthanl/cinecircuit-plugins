...
.maoyan-dialog__header{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:20px 24px}.maoyan-dialog__header h2{margin:0;font-size:22px;font-weight:600}
...
.maoyan-dialog__config{border-radius:50%!important;color:var(--app-on-accent)!important;background:var(--app-violet-text)!important}
...
@media(max-width:600px){.maoyan-dialog__content{padding:16px}.maoyan-dialog__header{padding:16px}.maoyan-dialog__header h2{font-size:18px}}
`;
...
export function install(sdk: CineCircuitPluginSdk) {
  ...
  const Statistics = defineComponent({
    name: "MaoyanStatistics", inheritAttrs: false,
    ...
    setup(props) {
      ...
      function overview() {
        ...
        const metrics = [
          ...
          { label: "来源平台", value: data.platform_count, unit: "个" },
        ];
        return h("section", { class: "maoyan-statistics", "aria-label": "猫眼榜单订阅统计" }, [
          h("dl", { class: "maoyan-statistics__metrics" }, metrics.map(metric => h("div", [h("dt", metric.label), h("dd", [String(metric.value), " ", h("span", metric.unit)])]))) ,
          data.total ? [
            h("div", { class: "maoyan-statistics__page-info", "aria-live": "polite" }, [h("span", `第 ${data.page} / ${data.pages} 页`), h("span", `本页 ${data.items.length} 条`)]),
            h("div", { class: "maoyan-statistics__grid" }, data.items.map(item => h("article", { class: "maoyan-subscription", key: item.id }, [
              h("div", { class: "maoyan-subscription__poster" }, posterUrl(item.poster) && !failedPosters.value.has(item.id) ? h("img", { src: posterUrl(item.poster), alt: `${item.title}海报`, loading: "lazy", onError: () => failedPosters.value.add(item.id) }) : h("span", "无海报")),
              h("div", { class: "maoyan-subscription__body" }, [h("h3", { title: item.title }, item.title), h("div", { class: "maoyan-subscription__tags" }, [h("span", item.media_label), h("span", { title: item.platform }, item.platform)]), h("p", { class: "maoyan-subscription__release" }, item.release_info || "暂无上线信息"), h("p", { class: "maoyan-subscription__time" }, ["订阅时间：", h("time", { datetime: item.subscribed_at }, subscriptionTime(item.subscribed_at))])]),
            ]))),
            data.pages > 1 ? h("nav", { class: "maoyan-statistics__pagination", "aria-label": "订阅记录分页" }, [h(Button, { variant: "text", size: "small", prependIcon: "mdi-chevron-left", disabled: data.page <= 1 || loading.value, onClick: () => load(data.page - 1) }, () => "上一页"), h("span", `${data.page} / ${data.pages}`), h(Button, { variant: "text", size: "small", appendIcon: "mdi-chevron-right", disabled: data.page >= data.pages || loading.value, onClick: () => load(data.page + 1) }, () => "下一页")]) : null,
          ] : h("div", { class: "maoyan-dialog__message" }, [h("h3", "暂无订阅记录"), h("p", "猫眼榜单中的作品成功加入订阅后，会显示在这里。")]),
        ]);
      }
      return () => h(Dialog, { modelValue: true, maxWidth: 1480, width: "calc(100vw - 32px)", "onUpdate:modelValue": (open: boolean) => { if (!open) props.context.close(); } }, () => h(Card, { class: "maoyan-dialog" }, () => [
        h("style", STYLE),
        h("header", { class: "maoyan-dialog__header" }, [h("h2", "猫眼榜单订阅"), h(Button, { icon: "mdi-close", variant: "text", "aria-label": "关闭", onClick: props.context.close })]),
        h("div", { class: "maoyan-dialog__content" }, loading.value ? h("p", { class: "maoyan-dialog__message", role: "status" }, "正在读取订阅统计…") : error.value ? h(Alert, { type: "error", variant: "tonal" }, () => error.value) : overview()),
        h("footer", { class: "maoyan-dialog__actions" }, [h(Button, { prependIcon: "mdi-refresh", variant: "text", loading: loading.value, onClick: () => load() }, () => "刷新"), h(Button, { class: "maoyan-dialog__config", icon: "mdi-cog-outline", variant: "flat", "aria-label": "配置猫眼榜单", title: "配置猫眼榜单", onClick: props.context.configure })]),
      ]));
    },
  });
