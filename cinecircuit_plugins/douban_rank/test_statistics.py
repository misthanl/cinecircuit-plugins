...
def test_history_counts_all_subscribed_records_and_keeps_board_metadata(tmp_path):
    ...
    for index in range(10):
        service.record_item(
            ...
            payload={
                "title": f"作品 {index}",
                "media_type": "movie" if index < 4 else "tv",
                "board": "热门综艺" if index == 9 else "一周口碑电影榜",
                "rating": "8.6",
                "poster": "https://img9.doubanio.com/poster.jpg",
    ...
    assert (first["total"], first["movie_count"], first["tv_count"], first["board_count"]) == (
        10,
        4,
        6,
        2,
...
def test_statistics_reads_all_gateway_pages_and_handles_missing_metadata():
    class Items:
        def list(self, limit, *, offset, status):
            ...
            return [
                {"id": index, "payload": {"media_type": "movie"}, "updated_at": ""}
                for index in range(offset, min(offset + limit, 501))
            ]
...
def test_plugin_api_handles_empty_history_and_frontend_stays_in_plugin(tmp_path):
    ...
    with pytest.raises(KeyError):
        asyncio.run(
            plugin.handle_api(PluginApiRequest(action="statistics", method="POST"), context)
        )
    built_root = tmp_path / "douban_rank"
    built_root.mkdir()
    (built_root / "frontend.js").write_text("export function install() {}", encoding="utf-8")
    path = _frontend_module_path(
        {
            "id": "douban-hot",
            "source": "zip",
            "enabled": True,
            "trusted": True,
            "install_path": str(built_root),
            "manifest": DoubanWatchlistPlugin.manifest.to_dict(),
        }
    )
    assert path is not None
    assert path.parent.name == "douban_rank"
    assert (
        _frontend_module_path(
            {"id": "douban-hot", "source": "builtin", "enabled": False, "trusted": True}
        )
        is None
    )
