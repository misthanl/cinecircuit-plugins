...
const { install } = await import("../../.build/cinecircuit_plugins/overseas_tv/frontend.js");
...
test("editor uses only host SchemaFields and preserves unrelated settings", async () => {
  let component;
  install({ vue, ui: { components: { SchemaFields: Fields } }, registerContribution() {}, registerEditor(entry) { component = entry.component; } });
  const fields = ["clear", "cron", "modes", "netflix_enabled", "netflix_num"].map(key => ({ key }));
  const wrapper = mount(component, { props: { modelValue: { netflix_enabled: false, netflix_num: "20", custom: "keep" } }, attrs: { fields } });
  const controls = wrapper.findAllComponents(Fields);
  assert.deepEqual(controls.slice(0, 2).map(c => c.props("fields")[0].key), ["clear", "cron"]);
  assert.equal(controls.at(-1).props("fields")[0].disabled, true);
  controls[2].vm.$emit("update:modelValue", { modes: ["returning"] });
  assert.equal(wrapper.emitted("update:modelValue").at(-1)[0].custom, "keep");
  assert.equal(wrapper.findAll("select,input,button").length, 0);
  assert(wrapper.classes().includes("plugin-config-grid"));
  wrapper.unmount();
});
...
test("editor exposes new platforms and shared type selector without removed settings", () => {
  let component;
  install({ vue, ui: { components: { SchemaFields: Fields } }, registerContribution() {}, registerEditor(entry) { component = entry.component; } });
  const keys = ["include_types", "apple_enabled", "apple_num", "amazon_enabled", "amazon_num", "hulu_enabled", "hulu_num", "preview", "exclude_ids", "animation", "documentary", "reality"];
  const wrapper = mount(component, { props: { modelValue: {} }, attrs: { fields: keys.map(key => ({ key })) } });
  assert.deepEqual(wrapper.findAllComponents(Fields).map(c => c.props("fields")[0].key), keys.slice(0, 7));
  assert(!wrapper.text().includes("不代表独家原创"));
  wrapper.unmount();
});
