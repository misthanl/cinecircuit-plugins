...
function statusLabel(item: RunItem) {
  return ({ preview: "仅预览", subscribed: "新增订阅", already_subscribed: "已订阅", duplicate: "已订阅", in_library: "已入库", skipped: "已跳过", unknown: "待重试", not_recognized: "待重试" } as Record<string, string>)[item.status] || "待重试";
}
  ...
  <UiDialog :model-value="true" :max-width="1000" width="calc(100vw - 32px)" @update:model-value="(open: boolean) => { if (!open) context.close(); }">
    <UiCard class="maoyan-dialog">
      ...
      <div class="maoyan-dialog__content">
        ...
        <section v-if="loaded" class="maoyan-statistics" aria-label="TMDB 剧集追踪统计">
          ...
          <div v-else class="maoyan-dialog__message"><h3>暂无订阅记录</h3><p>海外平台的剧集成功加入订阅后，会显示在这里。</p></div>
        </section>
      </div>
