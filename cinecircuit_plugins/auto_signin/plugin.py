...
class SiteCheckinPlugin(PluginBase):
    ...
    async def _retry_failed_sites(
        ...
        retry_state = self._daily_retry_state(context, today)
        retried_keys = set(retry_state["keys"])
        retry_rows = [
            row
            for row in results
            if not row.get("ok")
            ...
            and f"{row['mode']}:{row['site_id']}" not in retried_keys
        ]
        if retry_rows:
            retried_keys.update(f"{row['mode']}:{row['site_id']}" for row in retry_rows)
            state = getattr(context, "state", None)
            if state is not None:
                state.set(
                    "daily_keyword_retries",
                    {"day": today, "keys": sorted(retried_keys)},
                )
    ...
    @staticmethod
    def _daily_retry_state(context: PluginContext, today: str) -> dict[str, Any]:
        state = getattr(context, "state", None)
        saved = state.get("daily_keyword_retries", {}) if state is not None else {}
        if not isinstance(saved, dict) or saved.get("day") != today:
    ...
    def _notification_line(row: dict[str, Any]) -> str:
        mode = "登录" if row.get("mode") == "login" else "签到"
        fallback = (
            "登录状态正常"
        ...
        if request.action == "inventory" and request.method == "GET":
            ...
            return {
                **sites,
                ...
                "history": context.items.list(1000),
            }
