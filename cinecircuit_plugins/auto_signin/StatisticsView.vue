...
interface SiteItem { id: string; name: string; priority?: number }
...
const siteRows = computed(() => {
  ...
  return [...ids].map((id) => ({ id, name: findSiteName(id), records: history.value.filter((item) => String(item.payload?.site_id || "") === id && (item.payload?.mode || "sign") === "sign").length }))
    .sort((left, right) =>
      sitePriority(left.id) - sitePriority(right.id)
...
function sitePriority(id: string) {
...
<template>
  <UiDialog height="calc(100dvh - 48px)" :model-value="true" :max-width="972" width="calc(100vw - 32px)" @update:model-value="(open: boolean) => { if (!open) context.close(); }">
    <UiCard class="signin-stat-dialog" height="100%">
...
<style scoped>
.signin-stat-dialog{--stat-amber:#f5a524;--stat-red:#e45c72;--stat-blue:#2f93f1;--stat-violet:#7c64df;--stat-green:#39a96b;display:flex;height:calc(100dvh - 32px);max-height:calc(100dvh - 32px);flex-direction:column;overflow:hidden;border:1px solid var(--app-border-subtle);border-radius:20px!important;background:var(--app-surface);color:var(--app-text);box-shadow:0 28px 80px rgba(18,26,45,.22)}
...
.signin-stat-dialog__content{display:grid;flex:1 1 0;grid-template-rows:auto minmax(0,1fr);min-height:0;gap:20px;overflow:hidden;padding:22px 24px;background:var(--app-surface-subtle)}
