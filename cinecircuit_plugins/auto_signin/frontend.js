  ...
  name: "SiteCheckinPage",
    setup() {
      ...
      async function load() {
        loading.value = true;
        try {
          const data = await sdk.request(`/plugins/${PLUGIN_ID}/api/inventory`);
      ...
      async function runSite(site, mode) {
        ...
        try {
          const result = await sdk.request(`/plugins/${PLUGIN_ID}/api/site`, { method: "POST", body: JSON.stringify({ site_id: site.id, mode }) });
          ...
          sites.value.length ? h("div", { class: "signin-sites" }, sites.value.map((site) => {
            ...
            return h("article", { class: "signin-site" }, [
              ...
              h("div", { class: "signin-modes" }, [
                h("label", [h("input", { type: "checkbox", checked: signSites.value.includes(site.id), onChange: (event) => toggle(signSites, site.id, event.target.checked) }), "每日签到"]),
                h("label", [h("input", { type: "checkbox", checked: loginSites.value.includes(site.id), onChange: (event) => toggle(loginSites, site.id, event.target.checked) }), "保持登录"]),
