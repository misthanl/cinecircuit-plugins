...
function navigate(index:number){if(loading.value || props.disabled || index===folders.value.length-1)return;folders.value=folders.value.slice(0,index+1);search.value="";void load();}
 ...
 <section class="copy-browser" :aria-label="target ? '目标目录浏览器' : '源文件浏览器'">
  ...
  <div class="browser-files" :aria-busy="loading">
   <div v-for="item in visible" :key="item.file_id" class="browser-file" :class="{selected:selected.includes(String(item.file_id))}">
    ...
    <button v-if="item.directory" type="button" class="file-name" :disabled="loading || disabled" @click="enter(item)">{{item.name}}</button>
...
<style scoped>
.browser-search input,.browser-search input::placeholder{font-family:var(--app-font-family,"Microsoft YaHei",sans-serif)!important;font-size:var(--app-font-size-control,14px)!important;font-weight:var(--app-font-weight-regular,400)!important;line-height:1.5}
.browser-search input{background:transparent}
...
.browser-search input{color:var(--app-text);min-width:0;flex:1;outline:none;font-size:14px}.browser-search button{font-size:13px}.browser-search input::placeholder{color:var(--app-text-disabled)!important;opacity:1!important}.browser-file>.v-icon{color:var(--app-text-secondary)}
