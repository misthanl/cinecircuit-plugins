  ...
  name: "SubtitleWorkspacePage",
    setup() {
      ...
      async function load() {
        ...
        error.value = "";
        try {
          const result = await sdk.request(`/plugins/${PLUGIN_ID}/api/catalog?query=${encodeURIComponent(query.value)}&limit=100`);
      ...
      async function searchOnline() {
        ...
        try {
          ...
          const result = await sdk.request(`/plugins/${PLUGIN_ID}/api/online?query=${encodeURIComponent(keyword)}`);
      ...
      async function adjustSubtitle(subtitle) {
        ...
        try {
          const result = await sdk.request(`/plugins/${PLUGIN_ID}/api/adjust`, {
            method: "POST",
      ...
      h("div", [h("div", { class: "subtitle-kicker" }, "SUBTITLE WORKSPACE"), h("h1", "字幕管理助手"), h("p", "在线搜索、手动上传、同名匹配与外挂字幕管理。")]),
          h("div", { class: "subtitle-controls" }, [
            h("input", { class: "subtitle-search", value: query.value, placeholder: "搜索媒体名称或路径", onInput: (event) => { query.value = event.target.value; }, onKeyup: (event) => { if (event.key === "Enter") load(); } }),
        ...
        h("div", { class: "subtitle-layout" }, [
          ...
          h("main", { class: "subtitle-panel subtitle-detail" }, selected.value ? [
            ...
            h("div", { class: "subtitle-upload" }, [
              h("input", { type: "file", accept: ".srt,.ass,.ssa,.sub,.vtt,.webvtt", onChange: (event) => { file.value = event.target.files?.[0] || null; } }),
              h("select", { value: language.value, onChange: (event) => { language.value = event.target.value; } }, [["zh-CN", "简体中文"], ["zh-TW", "繁体中文"], ["zh", "中文"], ["en", "英语"], ["ja", "日语"], ["ko", "韩语"]].map(([value, label]) => h("option", { value }, label))),
