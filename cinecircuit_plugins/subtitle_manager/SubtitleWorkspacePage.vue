...
interface SubtitleFile { name: string; size?: number }
interface MediaItem { path: string; title?: string; subtitles?: SubtitleFile[] }
interface OnlineSubtitle { title: string; provider: string; language?: string; url?: string }
...
async function load() {
  ...
  error.value = "";
  try {
    const result = await props.request<ItemList<MediaItem>>(`/plugins/subtitle-manager/api/catalog?query=${encodeURIComponent(query.value)}&limit=100`);
...
async function searchOnline() {
  ...
  try {
    ...
    const result = await props.request<ItemList<OnlineSubtitle>>(`/plugins/subtitle-manager/api/online?query=${encodeURIComponent(keyword)}`);
...
async function removeSubtitle(subtitle: SubtitleFile) {
  ...
  try {
    ...
    await load();
  } catch (reason) { error.value = reason instanceof Error ? reason.message : "字幕删除失败"; }
}
...
async function adjustSubtitle(subtitle: SubtitleFile) {
  ...
  if (!Number.isFinite(offset)) { error.value = "请输入有效的时间偏移秒数"; return; }
  try {
    const result = await props.request<AdjustmentResult>("/plugins/subtitle-manager/api/adjust", {
      method: "POST",
    ...
    await load();
  } catch (reason) { error.value = reason instanceof Error ? reason.message : "字幕调轴失败"; }
}

function mediaName(item: MediaItem) { return item.title || item.path.split(/[\\/]/).pop(); }
function chooseFile(event: Event) { file.value = (event.currentTarget as HTMLInputElement).files?.[0] || null; }
function chooseLanguage(event: Event) { language.value = (event.currentTarget as HTMLSelectElement).value; }
function searchOnEnter(event: KeyboardEvent) { if (event.key === "Enter") load(); }
onMounted(load);
</script>

<template>
  <section class="subtitle-workspace">
    <header class="subtitle-hero">
      <div><div class="subtitle-kicker">SUBTITLE WORKSPACE</div><h1>字幕管理助手</h1><p>在线搜索、手动上传、同名匹配与外挂字幕管理。</p></div>
      <div class="subtitle-controls">
        <input class="subtitle-search" :value="query" placeholder="搜索媒体名称或路径" @input="query = ($event.currentTarget as HTMLInputElement).value" @keyup="searchOnEnter">
        <button class="subtitle-button" :disabled="loading" @click="load">{{ loading ? "读取中…" : "搜索" }}</button>
      </div>
    </header>
    <div class="subtitle-layout">
      <aside class="subtitle-panel">
        <h2>媒体目录</h2><p class="subtitle-panel-note">仅显示整理成功且当前可访问的本地媒体文件。</p>
        <div class="subtitle-list">
          <button v-for="item in items" :key="item.path" :class="['subtitle-media', selected?.path === item.path && 'is-active']" @click="selected = item">
            <span class="subtitle-rail"/><span><strong>{{ mediaName(item) }}</strong><small>{{ item.path }}</small></span><span class="subtitle-count">{{ item.subtitles?.length || 0 }} 条</span>
          </button>
          <div v-if="!items.length" class="subtitle-empty">{{ loading ? "正在读取媒体目录…" : "暂无可管理媒体" }}</div>
        </div>
      </aside>
      <main v-if="selected" class="subtitle-panel subtitle-detail">
        <div><h2>{{ selected.title || "字幕详情" }}</h2><p class="subtitle-path">{{ selected.path }}</p></div>
        <div class="subtitle-timeline">
          <div v-for="subtitle in selected.subtitles" :key="subtitle.name" class="subtitle-file">
            <strong>{{ subtitle.name }}</strong><small>{{ Math.max(1, Math.round((subtitle.size || 0) / 1024)) }} KB</small>
            <button class="subtitle-button" @click="adjustSubtitle(subtitle)">调轴</button><button class="subtitle-button" @click="removeSubtitle(subtitle)">删除</button>
          </div>
          <div v-if="!selected.subtitles?.length" class="subtitle-file"><span>尚未发现外挂字幕</span><small>可在线搜索或手动上传</small></div>
        </div>
        <div><button class="subtitle-button" :disabled="searching" @click="searchOnline">{{ searching ? "搜索中…" : "在线搜索字幕" }}</button></div>
        <div v-if="online.length" class="subtitle-timeline">
          <div v-for="item in online.slice(0, 20)" :key="`${item.provider}:${item.title}:${item.url || ''}`" class="subtitle-file">
            <a :href="item.url || '#'" target="_blank" rel="noopener noreferrer">{{ item.title }}</a><small>{{ item.provider }}{{ item.language ? ` · ${item.language}` : "" }}</small>
          </div>
        </div>
      ...
      <main v-else class="subtitle-panel subtitle-detail"><div class="subtitle-empty">请先从左侧选择媒体</div></main>
    </div>
  </section>
</template>
