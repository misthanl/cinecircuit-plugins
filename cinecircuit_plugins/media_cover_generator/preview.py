...
def _wedge_sample() -> bytes:
    ...
    sources = [
        p.read_bytes() for p in sorted((Path(__file__).parent / "assets").glob("source-*.jpg"))
    ][:3]
    return CoverRenderer().render_animated(
        sources,
        title="电影典藏",
        subtitle="MOVIE COLLECTION",
        item_count=0,
        options=CoverRenderOptions(style="wedge", width=1280, height=720, show_count=False),
        image_format="webp",
        duration_seconds=6,
...
def read_sample(style: str, variant: str) -> dict:
    if style in {"animated", "animated_diagonal", "animated_wedge"}:
        ...
        data = (
            _wedge_sample()
            if style == "animated_wedge"
            else (Path(__file__).parent / "assets" / name).read_bytes()
        )
        words = array.array("I")
    ...
    if style == "multi":
        ...
        sample_name = names.get(style)
    if sample_name is None:
        raise ValueError("未知封面样式")
    data = (Path(__file__).parent / "assets" / f"{sample_name}.jpg").read_bytes()
...
async def render_preview(plugin, config: dict, font_path: Path) -> dict:
    run = plugin.generation_run(SimpleNamespace(config=config))
    ...
    options = replace(
        run.options,
        width=max(1, round(run.options.width * factor)),
        height=max(1, round(run.options.height * factor)),
        jpeg_quality=94,
    )
    title, subtitle = "电影典藏", ""
    ...
    async with _semaphore:
        return await asyncio.to_thread(
            _render_sample, run, options, title, subtitle, config, font_path
        )


def _render_sample(run, options, title, subtitle, config, font_path):
    sources = [
        p.read_bytes() for p in sorted((Path(__file__).parent / "assets").glob("source-*.jpg"))
    ][: run.source_limit]
    ...
    return {
        ...
        "byte_length": len(data),
        "mime": mime,
        "width": common["options"].width,
        "height": common["options"].height,
    }
