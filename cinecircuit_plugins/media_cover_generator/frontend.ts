...
export function install(sdk: CineCircuitPluginSdk): void {
  ...
  sdk.registerEditor({ domain: "plugin", key: `${ID}:run`, component: defineComponent({
    ...
    emits:['update:modelValue'],
    setup(props,{attrs,emit}){return ()=>h(RunEditorView,{...attrs,modelValue:props.modelValue,disabled:props.disabled,request:sdk.request,cronFieldComponent:sdk.ui.components.CronField,schemaFieldsComponent:sdk.ui.components.SchemaFields,'onUpdate:modelValue':(value:Record<string,unknown>)=>emit('update:modelValue',value)});}
  }) });
}
