...
class CoverHistory:
    ...
    def import_latest(self, records, config):
        ...
        for record in records:
            ...
            self.add(
                data,
                {**result, "created_at": record.get("updated_at")},
                server,
                config,
                imported=token,
            )
    ...
    def path(self, identity, suffix):
        if not re.fullmatch("[0-9a-f]{32}", str(identity)) or suffix not in {
            "jpg",
            "png",
            "gif",
            "webp",
    ...
    def add(self, data, result, server, config, *, imported=None):
        with self.lock, self.db() as db:
            if (
                imported
                and db.execute("select 1 from imported where key=?", (imported,)).fetchone()
            ):
                return
            status = result.get("status", "preview")
            for key in ("generated", "uploaded" if status == "updated" else "preview"):
                db.execute(
                    "insert into counters values (?,1) on conflict(key) do update set value=value+1",
                    (key,),
                )
            token = (
                imported
                or f"{server}:{result.get('library_id')}:{hashlib.sha256(data).hexdigest()}"
            )
            ...
            self._prune_library(db, server, row, config)

    def _prune_library(self, db, server, row, config):
        limit = min(100, max(1, int(config.get("covers_history_limit_per_library") or 10)))
        siblings = sorted(
            [
                x
                for x in self.rows(db)
                if x["server_id"] == server and x["library_id"] == row["library_id"]
            ],
    ...
    def failure(self):
        with self.lock, self.db() as db:
            db.execute(
                "insert into counters values ('failed',1) on conflict(key) do update set value=value+1"
            )
    ...
    def read(self, identity, thumbnail=False):
        ...
        with self.lock, self.db() as db:
            ...
            return row, self.path(
                identity, "thumb.jpg" if thumbnail else row["suffix"]
            ).read_bytes()
    ...
    def applied(self, identity):
        with self.lock, self.db() as db:
            ...
            if row["status"] == "preview":
                db.execute(
                    "insert into counters values ('uploaded',1) on conflict(key) do update set value=value+1"
                )
