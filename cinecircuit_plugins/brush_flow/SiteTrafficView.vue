...
interface TrafficTask {
  ...
  delete_upload_gib?: number;
  delete_download_hours?: number;
  delete_inactive_hours?: number;
  delete_avg_upload_kib?: number;
  delete_task: boolean;
}
...
function add(): void {
  const task: TrafficTask = {
    ...
    delete_upload_gib: 0, delete_download_hours: 0, delete_inactive_hours: 0, delete_avg_upload_kib: 0,
...
<template>
  <section class="brush">
    <header class="brush-head app-surface-boundary">
      <div><h1>站点刷流</h1><p>任务、选种、下载和删种规则集中在同一个工作台。</p></div>
      <div class="brush-actions"><button class="ghost" :disabled="busy" @click="load">刷新</button><button class="app-save-button" :disabled="busy" @click="save"><VIcon icon="mdi-content-save-outline" />保存</button></div>
    </header>
    <div class="brush-layout">
      <aside class="brush-panel app-surface-boundary">
      ...
      <main class="brush-panel app-surface-boundary">
        <div v-if="selected" class="brush-form">
          ...
          <label class="brush-check"><input class="app-native-checkbox" type="checkbox" :checked="selected.enabled" @change="field('enabled', ($event.target as HTMLInputElement).checked)">启用任务</label>
          <label class="brush-field"><span>上传量达到后处理（GiB）</span><input type="number" min="0" :value="selected.delete_upload_gib || 0" @input="inputField('delete_upload_gib', $event, true)"></label>
          <label class="brush-field"><span>下载时长达到后处理（小时）</span><input type="number" min="0" :value="selected.delete_download_hours || 0" @input="inputField('delete_download_hours', $event, true)"></label>
          <label class="brush-field"><span>未活动时间达到后处理（小时）</span><input type="number" min="0" :value="selected.delete_inactive_hours || 0" @input="inputField('delete_inactive_hours', $event, true)"></label>
          <label class="brush-field"><span>平均上传速度低于时处理（KiB/s）</span><input type="number" min="0" :value="selected.delete_avg_upload_kib || 0" @input="inputField('delete_avg_upload_kib', $event, true)"></label>
          <label class="brush-check"><input class="app-native-checkbox" type="checkbox" :checked="selected.delete_task" @change="field('delete_task', ($event.target as HTMLInputElement).checked)">条件满足时删种（否则暂停）</label>
...
<style scoped>
.brush{display:grid;gap:18px;color:var(--app-text,#17243a)}.brush *{box-sizing:border-box}.brush-head,.brush-panel{border:1px solid var(--app-border,#dfe6f0);border-radius:22px;background:var(--app-surface,#fff);box-shadow:0 14px 36px rgba(38,60,94,.06)}.brush-head{display:flex;justify-content:space-between;align-items:center;gap:18px;padding:22px 24px}.brush h1,.brush h2{margin:0}.brush p{margin:5px 0 0;color:var(--app-text-muted,#71809a)}.brush-actions{display:flex;gap:9px}.brush button{min-height:40px;padding:0 15px;border:0;border-radius:12px;background:#2f74f6;color:#fff;font-weight:750;cursor:pointer}.brush button.ghost{background:var(--app-info-soft);color:var(--app-info-text)}.brush button.warn{background:var(--app-warning-soft);color:var(--app-warning-text)}.brush button:disabled{opacity:.5}.brush-layout{display:grid;grid-template-columns:minmax(240px,.65fr) minmax(0,1.45fr);gap:16px}.brush-panel{padding:20px}.brush-list{display:grid;gap:9px;margin-top:15px}.brush-task{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;padding:13px;border:1px solid var(--app-border,#dfe6f0);border-radius:14px;background:var(--app-surface-subtle);cursor:pointer}.brush-task.on{border-color:#82adff;background:var(--app-choice-active-bg)}.brush-task strong,.brush-task small{display:block}.brush-task small{margin-top:4px;color:var(--app-text-muted)}.brush-form{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:13px}.brush-field{display:grid;gap:6px}.brush-field.full{grid-column:1/-1}.brush-field span{font-size:13px;font-weight:700;color:var(--app-text-secondary)}.brush-field input,.brush-field select{width:100%;min-height:42px;padding:0 12px;border:1px solid #d6deea;border-radius:11px;background:var(--app-surface)}.brush-check{display:flex;gap:8px;align-items:center;min-height:42px}.brush-preview{grid-column:1/-1;display:grid;gap:7px;padding-top:13px;border-top:1px solid #e3e8f1}.brush-preview div{padding:9px 11px;border-radius:10px;background:var(--app-surface-subtle);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.brush-empty{padding:30px;text-align:center;color:var(--app-text-muted)}@media(max-width:800px){.brush-layout{grid-template-columns:1fr}.brush-form{grid-template-columns:1fr}.brush-field.full{grid-column:auto}.brush-head{align-items:stretch;flex-direction:column}.brush-actions{display:grid}}
</style>
